...
func InitializeConnection() (bool, string, mig.Connection) {
	...
	dbHost := config.GetEnvOrDefault("AGENDA_DB_HOST", "")
	if host != "" {
		dbHost = host
	}
	dbDB := config.GetEnvOrDefault("AGENDA_DB_DB", "")
	if db != "" {
		dbDB = db
	}
	dbUser := config.GetEnvOrDefault("AGENDA_DB_USER", "")
	if user != "" {
		dbUser = user
	}
	dbPass := config.GetEnvOrDefault("AGENDA_DB_PASS", "")
	if pwd != "" {
		dbPass = pwd
	}
	adminDBUser := config.GetEnvOrDefault("AGENDA_ADMIN_DB_USER", "")
	if adminUser != "" {
		adminDBUser = user
	}
	adminDBPass := config.GetEnvOrDefault("AGENDA_ADMIN_DB_PASS", "")
	if adminPwd != "" {
		adminDBPass = pwd
	}
	dbEngine := config.GetEnvOrDefault("AGENDA_MIGRATION_DB_ENGINE", "")
	if engine != "" {
		dbEngine = engine
	}
