...
func (m *ManagerAgenda) printProgramProgram(pdfP *gofpdf.Fpdf, pdfL *gofpdf.Fpdf, agenda *Agenda) {
	...
	pdfL.Cell(81, 5, invocation)
	pdfL.Cell(20, 5, "")
	pdfL.Cell(38, 5, "Invocation:")
	pdfL.Cell(0, 5, invocation)
	pdfP.Ln(5)
	pdfL.Ln(5)
	if agenda.WardBusiness.Bool {
		pdfP.Ln(2)
	...
	if agenda.Fastsunday.Bool {
		...
		if agenda.FSMusicalNumber.String != "" {
			...
			pdfL.Cell(4, 5, "")
			resetY := pdfL.GetY()
			pdfL.MultiCell(115, 5, fmt.Sprintf("Musical Number: %s", agenda.FSMusicalNumber.String), "", "", false)
			pdfL.SetXY(153, resetY)
			// pdfL.Cell(24, 5, "")
			pdfL.MultiCell(0, 5, fmt.Sprintf("Musical Number: %s", agenda.FSMusicalNumber.String), "", "", false)
			pdfL.Ln(7)
		}
		pdfP.Cell(4, 5, "")
	...
	pdfL.Cell(81, 5, benediction)
	pdfL.Cell(20, 5, "")
	pdfL.Cell(38, 5, "Benediction:")
	pdfL.Cell(0, 5, benediction)
	pdfP.Ln(8)
	pdfL.Ln(8)
}
